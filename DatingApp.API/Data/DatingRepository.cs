...
namespace DatingApp.API.Data
{
	public class DatingRepository : IDatingRepository
	{
		...
		public async Task<PagedList<User>> GetUsers(UserParams userParams)
		{
			...
			//filters
			//get all except the current user
			users = users.Where(u => u.Id != userParams.UserId);

			//gets the gender as specify in user paramenter or as query
			users = users.Where(u => u.Gender == userParams.Gender);

			if (userParams.Likers)
			{
				// get users with likers
			...
			{
				// get users with likee (userParams.Likers is either true or false)
		...
		{
			//get user with his likers and likees
			var user = await _context.User
			.Include(x => x.Likee)
			.Include(x => x.Liker)
			.FirstOrDefaultAsync(u => u.Id == id);

			//return the likees or the likers
			if (likers)
			{
		...
		public async Task<User> GetUser(int id, bool isCurrentUser)
		{
			...
			// checksif its the current user, true =>  ignores globalqueryfilters
			if (isCurrentUser)
				query = query.IgnoreQueryFilters();

			//return the photos with the user
		...
		{
			//return true if there one or more stuff to save 
			return await _context.SaveChangesAsync() > 0;
		}

		public async Task<Photo> GetPhoto(int id)
		{
			// so can be able to set main photo and also
		...
		//checks if the user already likes the recipient
		...
		public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
		{
			...
			switch (messageParams.MessageContainer)
			{
				case "inbox":
					...
					break;
				case "Outbox":
					messages = messages.Where(u => u.SenderId == messageParams.UserId && !u.SenderDelete);
					break;
				default:
